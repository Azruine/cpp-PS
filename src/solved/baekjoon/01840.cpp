...
class DancingLinks {
    ...
    std::array<size_t, BOARD> row_mask = {};
    std::array<size_t, BOARD> col_mask = {};
    std::array<size_t, BOARD> blk_mask = {};

    static size_t get_blk(size_t row, size_t col) {
...
class Solver {
    ...
    bool write(size_t row, size_t col, size_t num) {
        if (board[(row * BOARD) + col] != 0) {
            return false;
        }
        auto blk = get_blk(row, col);
        auto bit = 1U << (num - 1);
        if (col_mask[col] & bit) {
            return false;
        }
...
public:
    void solve() {
        ...
        size_t row, col, num, cnt = 0;
        while (cin >> row >> col >> num) {
            cnt++, row--, col--;
            if (!write(row, col, num)) {
                cout << cnt;
                return;
            }
            dlx = DancingLinks();
