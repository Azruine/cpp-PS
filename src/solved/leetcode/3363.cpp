...
public:
    int maxCollectedFruits(std::vector<std::vector<int>>& fruits) {
        ...
        for (auto col : _iota(1, n - 1)) {
            for (auto row : _iota(n - 1 - col, n)) {
                dp[row][col] = fruits[row][col];
                auto prev = dp[row][col - 1];
                if (row < n - 1) {
                    prev = std::max(prev, dp[row + 1][col - 1]);
                }
                if (row > 0) {
                    prev = std::max(prev, dp[row - 1][col - 1]);
                }
        ...
        for (auto row : _iota(1, n - 1)) {
            for (auto col : _iota(n - 1 - row, n)) {
                dp[row][col] = fruits[row][col];
                auto prev = dp[row - 1][col];
                if (col < n - 1) {
                    prev = std::max(prev, dp[row - 1][col + 1]);
                }
                if (col > 0) {
...
int main() {
    ...
    cout << sol.maxCollectedFruits(fruits) << '\n';  // Example usage
}
