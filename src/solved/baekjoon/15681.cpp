...
class Solver {
    size_t m_root = 0, m_size = 0, m_queries = 0;
    ...
    void read() {
        cin >> m_size >> m_root >> m_queries;
        m_adj.resize(m_size + 1);
    ...
    void construct() {
        m_tree.assign(m_size + 1, Node{});
        m_tree[m_root].parent = 0;
        m_tree[m_root].depth = 0;
        ...
        stack.push_back({m_root, 0, false});

        while (!stack.empty()) {
            auto [node, parent, exiting] = stack.back();
            stack.pop_back();
            if (exiting) {
                m_tree[node].subtree_size = 1;
                for (size_t child : m_tree[node].children) {
                    m_tree[node].subtree_size += m_tree[child].subtree_size;
                }
                continue;
            }

            stack.push_back({node, parent, true});
            for (auto child : m_adj[node]) {
                if (child != parent) {
                    m_tree[node].children.push_back(child);
                    m_tree[child].parent = node;
                    m_tree[child].depth = m_tree[node].depth + 1;
                    stack.push_back({child, node, false});
                }
            }
        }
    }
