...
class Solver {
    ...
    int find_min() {
        std::array<int, 3> cur_min = {0, 0, 0};
        std::array<int, 3> next_min = {0, 0, 0};
        cur_min = {INT32_MAX, nodes[1], nodes[1] + nodes[2]};
        for (auto i : iota(1, n)) {
            next_min.fill(INT32_MAX);
            next_min[0] = std::min(cur_min[0], cur_min[1]) + nodes[(3 * i) + 0];
            next_min[1] =
                std::min({next_min[0], cur_min[0], cur_min[1], cur_min[2]}) +
                nodes[(3 * i) + 1];
            next_min[2] = std::min({cur_min[1], cur_min[2], next_min[1]}) +
                          nodes[(3 * i) + 2];
            std::swap(cur_min, next_min);
        }
        return cur_min[1];
    }
...
public:
    void solve() {
        while (read()) {
            auto result = find_min();
            std::println("{}. {}", ++t, result);
        }
    }
};
