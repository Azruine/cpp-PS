...
class Solver {
    ...
    int32_t part_1() {
        ...
        for (auto& row : map) {
            int32_t split = 0;
            for (auto const& idx : prev) {
                // fill straight drop down
                if (row[idx] == '.') {
                    row[idx] = '|';
                }
                // split
                if (row[idx] == '^') {
                    // boundary check not required, good input data...
                    // always have space between splitter
                    row[idx - 1] = '|', row[idx + 1] = '|';
                    split += 1;
                }
            }
            ret += split;
            for (auto i : iota(cols)) {
                if (row[i] == 'S' || row[i] == '|') {
                    cur.push_back(i);
                }
            }
    ...
    int64_t part_2() {
        std::vector<size_t> prev, cur;
        std::vector dp(rows, std::vector(cols, 0L));
        for (auto i : iota(cols)) {
            if (map[0][i] == 'S') {
                dp[0][i] = 1;
                break;
        ...
        for (auto row : iota(rows)) {
            for (auto const& idx : prev) {
                // add straight drop down
                if (map[row][idx] != '^') {
                    map[row][idx] = '|';
                    dp[row][idx] += dp[row - 1][idx];
                }
                // split
                if (map[row][idx] == '^') {
                    // boundary check not required, good input data...
                    // always have space between splitter
                    map[row][idx - 1] = '|', map[row][idx + 1] = '|';
                    dp[row][idx - 1] += dp[row - 1][idx];
                    dp[row][idx + 1] += dp[row - 1][idx];
                }
            }
            for (auto i : iota(cols)) {
        ...
        return std::ranges::fold_left(dp.back(), 0L, std::plus{});
    }
