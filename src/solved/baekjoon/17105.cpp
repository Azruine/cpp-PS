...
class Solver {
    ...
    size_t t = 0;
    uint32_t n = 0;
    constexpr static uint32_t MAX = 1'000'010;
    constexpr static uint32_t MAX_2 = []() {
        uint32_t res = 1;
        while (res < MAX * 3 / 2) {
            res <<= 1;
        }
    ...
    sieve::LinearSieve<uint32_t> sieve{MAX};
    std::vector<bool> is_prime = sieve.get_is_prime();
    std::vector<complex> is_prime_cpx, is_prime_cpx_2;
    void set_cpx_vec() {
        is_prime_cpx.resize(MAX_2);
        is_prime_cpx_2.resize(MAX_2);
        for (size_t i = 3; i < MAX; i++) {
            if (is_prime[i]) {
                auto k = i >> 1;
                is_prime_cpx[k] = complex(1, 0);
                if (k < MAX_2) {
                    is_prime_cpx_2[k * 2] = complex(1, 0);
                }
            }
        }
        fft::fft(is_prime_cpx);
        fft::fft(is_prime_cpx_2);
    }
    void multi() {
        for (size_t i = 0; i < MAX_2; i++) {
            is_prime_cpx_2[i] *= is_prime_cpx[i];
            is_prime_cpx[i] *= (is_prime_cpx[i] * is_prime_cpx[i]);
        }
        fft::fft(is_prime_cpx, true), fft::fft(is_prime_cpx_2, true);
    }
    uint32_t cnt(size_t x) {
        uint32_t ret = 0;
        if (x - 4 > 2 && is_prime[x - 4]) {
            ret += 1;
        }
        uint32_t target = (x - 3) / 2;
        auto diff_tuple =
            as<uint32_t>(std::lround(is_prime_cpx[target].real()));
        auto same_tuple =
            as<uint32_t>(std::lround(is_prime_cpx_2[target].real()));
        auto triple = (x % 3 == 0 && is_prime[x / 3]);
        ret += (diff_tuple + 3 * same_tuple + 2 * triple) / 6;
        return ret;
    }
