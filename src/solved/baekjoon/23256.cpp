...
class Solver {
    ...
    std::vector<int64_t> dp_1, dp_2;

    void fill() {
        dp_1.assign(MAX, 0), dp_2.assign(MAX, 0);
        dp_1[0] = 1, dp_1[1] = 7, dp_1[2] = 33;
        dp_2[0] = 1, dp_2[1] = 3, dp_2[2] = 13;
        for (size_t i = 3; i < MAX; i++) {
            dp_1[i] = (dp_1[i - 1] * 3 + dp_2[i - 1] * (7 - 3)) % MOD;
            dp_2[i] = (dp_1[i - 1] * 1 + dp_2[i - 1] * (3 - 1)) % MOD;
        }
    }
